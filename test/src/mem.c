...
TESTCASE(phys_read_aligned, {
    ...
    data = 0;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x10080, &data, 0, RV_ACCESS_LOAD, false) == RV_MEM_OK);
    TEST_ASSERT(data == 0xAB);

    data = 0;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x10100, &data, 1, RV_ACCESS_LOAD, false) == RV_MEM_OK);
    TEST_ASSERT(data == 0x1234);

    data = 0;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x10200, &data, 2, RV_ACCESS_LOAD, false) == RV_MEM_OK);
    TEST_ASSERT(data == 0x12345678);

    data = 0;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x10300, &data, 3, RV_ACCESS_LOAD, false) == RV_MEM_OK);
...
TESTCASE(phys_write_aligned, {
    ...
    data = 0xAB;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x10080, &data, 0, RV_ACCESS_STORE, false) == RV_MEM_OK);
    TEST_ASSERT(machine->ram[0x080] == 0xAB);

    data = 0x1234;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x10100, &data, 1, RV_ACCESS_STORE, false) == RV_MEM_OK);
    TEST_ASSERT(machine->ram[0x100] == 0x34);
    TEST_ASSERT(machine->ram[0x101] == 0x12);

    data = 0x12345678;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x10200, &data, 2, RV_ACCESS_STORE, false) == RV_MEM_OK);
    ...
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x10300, &data, 3, RV_ACCESS_STORE, false) == RV_MEM_OK);
...
TESTCASE(phys_read_misaligned, {
    ...
    uint64_t data       = 0;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x10001, &data, 1, RV_ACCESS_LOAD, false) == RV_MEM_OK);
    ...
    data                = 0;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x10012, &data, 2, RV_ACCESS_LOAD, false) == RV_MEM_OK);
    TEST_ASSERT(data == 0x12345678);
    ...
    data                = 0;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x10024, &data, 3, RV_ACCESS_LOAD, false) == RV_MEM_OK);
    TEST_ASSERT(data == 0x0102030405060708);
})
...
TESTCASE(phys_write_misaligned, {
    ...
    // Half at odd address (addr % 2 == 1).
    data = 0x1234;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x10001, &data, 1, RV_ACCESS_STORE, false) == RV_MEM_OK);
    TEST_ASSERT(machine->ram[0x001] == 0x34);
    TEST_ASSERT(machine->ram[0x002] == 0x12);

    // Word at 2-byte-aligned address (addr % 4 == 2).
    data = 0x12345678;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x10012, &data, 2, RV_ACCESS_STORE, false) == RV_MEM_OK);
    ...
    // Dword at 4-byte-aligned address (addr % 8 == 4).
    data = 0x0102030405060708;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x10024, &data, 3, RV_ACCESS_STORE, false) == RV_MEM_OK);
...
TESTCASE(phys_fault_load_outside, {
    uint64_t data = 0;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x08000, &data, 3, RV_ACCESS_LOAD, false) == RV_MEM_ACCESS_FAULT);
})

TESTCASE(phys_fault_store_outside, {
    uint64_t data = 0xDEAD;
    TEST_ASSERT(rv_access_phys(machine, cpu, 0x20000, &data, 3, RV_ACCESS_STORE, false) == RV_MEM_ACCESS_FAULT);
})
...
TESTCASE(phys_fault_load_edge, {
    uint64_t data = 0;
    TEST_ASSERT(
        rv_access_phys(machine, cpu, machine->ram_start - 4, &data, 3, RV_ACCESS_LOAD, false) == RV_MEM_ACCESS_FAULT
    );
})
...
TESTCASE(phys_fault_store_edge, {
    uint64_t data = 0;
    TEST_ASSERT(
        rv_access_phys(machine, cpu, machine->ram_end - 4, &data, 3, RV_ACCESS_STORE, false) == RV_MEM_ACCESS_FAULT
    );
})
