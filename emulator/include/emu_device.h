...
typedef bool (*emu_mmio_read_fn_t)(void *dev, uint64_t offset, uint8_t size, uint64_t *out);
typedef bool (*emu_mmio_write_fn_t)(void *dev, uint64_t offset, uint8_t size, uint64_t value);
...
struct emu_mmio_region {
    ...
    uint64_t            size;
    void               *device;
    emu_mmio_read_fn_t  read;
    emu_mmio_write_fn_t write;
};
